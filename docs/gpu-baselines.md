...
2026-09-08 增加 `natural`、`ridges`、`cells` 三种连续 scalar profile，以及仅由世界坐标
驱动的多方向低频场。旧预设的世界细节振幅为 0，渲染结果保持不变；新预设使用该场
跨越多个 tile 组织宏观形态，局部周期基底只承担细节。

| 10 x 10 新预设 | 最大标量差 | 最大线性 RGB 差 | 最大 GPU 残差 | 最大 8-bit 差 |
| --- | ---: | ---: | ---: | ---: |
| `tiling_graphic_screenprint_10x10` | `6.660e-5` | `8.439e-4` | `4.998e-7` | 1 |
| `tiling_ink_wash_10x10` | `5.434e-5` | `6.188e-5` | `4.999e-7` | 1 |
| `tiling_cellular_camo_10x10` | `1.010e-4` | `3.637e-4` | `4.998e-7` | 1 |

三套均无 CPU/GPU 逆映射失败、无无效 GPU 像素，且没有超过 1 LSB 的像素。2880 x 2880
视觉检查确认它们分别呈现平面原色色域、低对比水墨云团和闭合迷彩岛，不再复用原三套
预设的等高线浮雕语言，也未发现明显的 10 x 10 网格边界。

同设备 720 x 720、240 帧同步冒烟测量中，旧 QRP 预设为 2.533 ms/帧，新丝网印刷预设
为 2.055 ms/帧。后者关闭轮廓和浮雕，因此该数字只用于确认新增世界场没有造成异常
性能回退，不用于比较风格优劣。
