...
驱动的多方向低频场。随后完成风格精修：丝网印刷采用四级软色阶，水墨使用 Wang
边颜色约束的三次曲线配对并叠加轻微世界颗粒，迷彩使用带区域身份的 Worley 胞元。
旧预设的边结构振幅为 0，渲染结果与成本语义保持不变。

| 10 x 10 新预设 | 最大标量差 | 最大线性 RGB 差 | 最大 GPU 残差 | 最大 8-bit 差 |
| --- | ---: | ---: | ---: | ---: |
| `tiling_graphic_screenprint_10x10` | `4.416e-5` | `9.078e-4` | `4.998e-7` | 1 |
| `tiling_ink_wash_10x10` | `8.720e-5` | `1.159e-4` | `4.999e-7` | 1 |
| `tiling_cellular_camo_10x10` | `1.738e-5` | `4.179e-5` | `4.998e-7` | 1 |

三套均无 CPU/GPU 逆映射失败、无无效 GPU 像素，且没有超过 1 LSB 的像素。2880 x 2880
视觉检查确认它们分别呈现连续大色块套印、跨 tile 平滑连接的墨线路径和多色胞元迷彩，
不再复用原三套预设的等高线浮雕语言。水墨路径包含弧线、闭环与交叉，但边界入口和
线宽只由共享 Wang 边颜色决定；未发现断线、尖钩或显式的 10 x 10 方格边界。样张保存在
被 Git 忽略的 `output/`，便于本地复核而不污染源码历史。

同设备 720 x 720、300 帧同步冒烟测量中，带 16 段三次曲线求值的水墨预设为
5.392 ms/帧（185.475 fps）。Wang 曲线和 Worley 的条件分支都仍处于实时预算内；不同
