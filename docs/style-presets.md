...
多方向世界场，`worldGrainAmplitude` 提供连续的全局细节，`edgeStructureAmplitude`
控制由 Wang 边颜色直接定义的连接笔画。胞元风格同样直接从世界坐标确定，因此不会在
tile 边界重新播种。它们都复用同一套几何、预览与导出路径。
...
| 7 | `tiling_ink_wash_10x10` | Wang 边颜色决定入口与线宽，tile seed 选择内部三次曲线配对 | 暖纸底深墨线、轻微世界颗粒、无浮雕 |
...
- CPU 与 GLSL 使用相同的 profile、世界颗粒、胞元哈希、边配对曲线和软色阶公式。
- profile 测试覆盖 natural、ridges、cells，并在非零世界细节下检查共享边连续性。
- 边结构测试以不同 tile seed 和非共享边颜色检查横向、纵向共享边的精确一致性。
- 新预设分别通过 CPU/GPU 中间量及最终 8-bit 图像对照。
- PNG 与 JSON 继续记录 committed 参数；JSON 的生成器模型版本为 `hybrid_torus_v4`，
  并显式保存 scalar profile、世界细节、世界颗粒、胞元尺度、边结构振幅与色阶参数。
